...
def get_dataset(args, is_train, max_txt_length=64, epoch_id=0):
    ...
    ### 修改
    dataloader = DataLoader(dataset, batch_size=batch_size, pin_memory=False, num_workers= args.num_workers if is_train else 1, sampler=None)
    dataloader.num_samples = num_samples
    assert num_samples % dataset.global_batch_size == 0
    dataloader.num_batches = num_samples // dataset.global_batch_size

    return DataInfo(dataloader=dataloader, sampler=None, dataset=dataset, epoch_id=epoch_id)
